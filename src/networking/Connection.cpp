...
void Connection::receiveThread()
{
    while (m_running.load())
    {
        ...
        int bytesRead = recv(m_socket,
                             ...
                             static_cast<int>(RECV_BUFFER_SIZE - m_recvBufferPos),
                             0);
        ...
        else
        {
            ...
            // Try to extract as many packets as possible from the buffer
            while (m_running.load() && m_recvBufferPos >= sizeof(uint8_t))
            {
